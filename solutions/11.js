...
// Another DP problem. This one would be really fun in python!

const transpose = require('../helpers/partialSums')
...
function findMaxProduct(matrix, len) {
  ...
  const diagMax = getDiagonals(matrix).map(r => maxProduct(r, len))
  return rowMax.concat(colMax).concat(diagMax).filter(n => !!n).reduce((l, r) => Math.max(l, r), -Infinity)
}
